    ...
    def __init__(self,
        ...
        if skip_feat:  # skip_feat=True
            self.skip_feat = nn.Linear(
                self._in_src_feats, self._out_feats*self._num_heads, bias=bias)  # (输入：126，输出：64*4)
        else:
            self.skip_feat = None
        if gated:  # gated=True
            self.gate = nn.Linear(
                3*self._out_feats*self._num_heads, 1, bias=bias)  # (输入：3*64*4=768，输出：1）
        else:
            self.gate = None
...
# 公式4
        if self.skip_feat is not None:  # skip_feat=True
            ...
            # feat[:graph.number_of_dst_nodes() ={Tensor:(681,126)}
            # self.skip_feat (输入：126，输出：64*4）
...
class GraphAttnModel(nn.Module):
    def __init__(self,
        ...
        for l in range(0, (self.n_layers - 1)):
            ...
            self.layers.append(TransformerConv(in_feats=self.hidden_dim * self.heads[l - 1],  # self.heads[l - 1]即self.heads[- 1]=4
