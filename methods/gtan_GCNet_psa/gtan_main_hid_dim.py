...
from .gtan_model_GCNet_psa import GraphAttnModel
from tqdm import tqdm
from .my_add import plot_results


def gtan_main_hid_dim(feat_df, graph, train_idx, test_idx, labels, args, cat_features):
    hid_dims = [8, 16, 32, 64, 128, 256, 512, 1024]  # 示例值，实际值根据你的实验设置
    ...
    for i in hid_dims:
                    ...
                    for step, (input_nodes, seeds, blocks) in enumerate(
                        ...
                        if step % 10 == 0:
                            ...
                            try:
                                ...
                                pass
                val_losses.append(val_loss_list / val_all_list)
                val_accuracies.append(val_acc_list / val_all_list)
        ...
        auc = roc_auc_score(y_target, test_score)
        f1 = f1_score(y_target, test_score1, average="macro")
        ap = average_precision_score(y_target, test_score)

        results['hid_dim'].append(i)
        ...
        # 绘制训练和验证损失/准确率曲线
        # plot_loss_accuracy(train_losses, val_losses, train_accuracies, val_accuracies, f'fold_{fold + 1}_loss_accuracy.png')

        # 关闭文件
        output_file.close()
        # 在所有实验完成后，调用绘图函数
    print(results)
    plot_results(results, 'hid_dim_experiment.png')
