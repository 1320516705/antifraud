...
#
#         # 使用相同的激活函数（如 ReLU）
#         self.relu = nn.ReLU()  # 统一使用 ReLU 激活函数
#
#         # 最终的融合层，输出特征维度为 out_feats
#         self.fc2 = nn.Linear(out_feats // self.S, out_feats)
#
...
#         h_split = torch.chunk(h, self.S, dim=1)  # h_split 是一个包含 S 个张量的列表，每个张量形状为 [batch_size, in_feats // S]
#
#         # Step 2: 对每个子空间进行独立处理，应用相同的全连接层和激活函数
#         weights = []
#         for i in range(self.S):
#             # 对每个子空间应用独立的全连接层
#             fc_out = self.fc_layers[i](h_split[i])  # fc_out 形状为 [batch_size, out_feats // S]
#
#             # 对每个子空间应用相同的激活函数
#             fc_out = self.relu(fc_out)  # fc_out 形状为 [batch_size, out_feats // S]
#
#             # 使用 Softmax 计算注意力权重
#             weight = self.softmax(fc_out)  # weight 形状为 [batch_size, out_feats // S]
#             weights.append(weight)
...
class GTANWithPSA(nn.Module):
    def __init__(self, in_feats, out_feats):
        ...
        self.S = get_max_s(in_feats)  # 分成 S 个子空间（建议从4或6开始）
    ...
    def forward(self, h):
        ...
        """
        # Step 1: 将输入特征 h 划分成 S 个子空间
        h_split = torch.chunk(h, self.S, dim=1)
        ...
        for i in range(self.S):
            ...
            weights.append(fc_out)

        # Step 2: 在加权融合之后计算 softmax
        h_fused = sum(weights)  # h_fused 形状为 [batch_size, in_feats // S]
        h_fused = self.fc2(h_fused)  # 通过全连接层得到最终特征表示

        # Softmax 计算权重
        h_fused = self.softmax(h_fused)
